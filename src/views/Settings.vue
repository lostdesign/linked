  ...
  <Layout>
    <main class="px-10 mt-10 text-black dark:text-white">
      <router-link to="/" class="flex items-center align-center mb-3 hover:text-red-500">
        <span class="mr-1"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"><path
      ...
      <div class="flex space-x-5 mb-4">
        <template v-for="(lang, index) in languages">
          <div
            ...
            :class="{'bg-gray-800 text-white' : index !== language, 'bg-gray-100 text-black': index === language}"
      ...
      <span
        class="fixed bottom-0 left-0 w-full p-5 text-center text-white self-end text-xs text-gray-500">v{{
          version
        }}</span>
    </main>
  </Layout>
...
export default {
  data() {
    return {
      ...
      set language(lang) {// eslint-disable-line
