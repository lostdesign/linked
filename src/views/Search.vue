    ...
    <main class='px-10 mt-10 text-black dark:text-white'>
      <div class='sticky top-0 bg-white dark:bg-black -mt-4 -m-0.5'>
        <div class='flex justify-between w-full pt-4'>
          ...
          <div v-if="searchResults.length >= 1" class='text-xs text-gray-600 dark:text-gray-300'>{{ searchResults.length || 0 }} {{ $t('search.result_amount') }}</div>
        </div>
        <input
          type='text'
          name='search'
      ...
      <div v-if='_searchSomething' class='flex flex-col justify-center items-center mt-24'>
        ...
        <div class='mt-8 w-full text-center text-sm' v-html="$t('search.start_searching')"></div>
      </div>
      <div v-else-if='_noResults' class='flex flex-col justify-center items-center mt-24'>
        <nothing-found class='w-2/3'/>
        <div class='mt-8 w-full text-center text-sm' v-html="$t('search.no_result', {searchTerm})"></div>
      </div>
      <div v-else>
        <div 
          ref='results'
